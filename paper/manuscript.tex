...
\begin{proof}
Every positive quadratic $x_i^2-a_i(j)$ splits into two distinct real linear factors. Since the factors use independent private variables, $K_j$ is the intersection of $2^7$ real maximal ideals. The sign analysis in the proof of Theorem~\ref{thm:counterexample} shows that these nine fibers are exactly the real common-zero set of $I$. The real Nullstellensatz gives the displayed decomposition.
\end{proof}
...
\]
Successive division by the nine monic private-variable quadratics gives the explicit $\C[t]$-basis
\[
